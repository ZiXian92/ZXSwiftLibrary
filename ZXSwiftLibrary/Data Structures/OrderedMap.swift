...
/**
    Defines an ordered collection of mapping of values to keys.
    Ordering is based on the defined natural ordering of the keys.
    Implementation is an AVL tree.
 */
public class OrderedMap<K: Comparable, V> {
    private var _root: OrderedMapNode<K, V>?
    ...
    /**
        The smallest key in the collection.
     */
    public var smallestKey: K? {
        return isEmpty ? nil: minElement(_root!).key
    }
    
    /**
        The value mapped to the smallest key.
     */
    public var valueForSmallestKey: V? {
        return isEmpty ? nil: minElement(_root!).value
    }
    
    /**
        The largest key in the colleciton.
     */
    public var largestKey: K? {
        return isEmpty ? nil: maxElement(_root!).key
    }
    
    /**
        The value mapped to the largest key.
     */
    public var valueForLargestKey: V? {
    ...
    /**
        Creates a new empty collection of key-value pairs.
     */
    public init() {
        _root = nil
        _size = 0
     ...
     */
    public func insertValueForKey(key: K, value: V) {
        if isEmpty {
            _root = OrderedMapNode<K, V>(key: key, value: value, parent: nil)
            _size++
        } else {
            insert(OrderedMapNode<K, V>(key: key, value: value, parent: nil), curNode: _root!)
        }
    }
    
    //The recursive insertion function
    private func insert(newNode: OrderedMapNode<K, V>, curNode: OrderedMapNode<K, V>) {
        //Catch duplicate
    ...
    private func rotateLeft(node: OrderedMapNode<K, V>) {
    ...
    private func rotateRight(node: OrderedMapNode<K, V>) {
    ...
    private func containsKey(key: K, curNode: OrderedMapNode<K, V>) -> Bool {
        if key == curNode.key {
            return true
        }
    ...
    private func valueForKey(key: K, curNode: OrderedMapNode<K, V>) -> V? {
        if key == curNode.key {
     ...
     */
    public func eraseValueForKey(key: K) {
        if isEmpty {
            return
        }
    ...
    private func erase(key: K, curNode: OrderedMapNode<K, V>) {
        var node: OrderedMapNode<K, V>
        if key < curNode.key {
            if !curNode.hasLeftChild {
                return
            }
    ...
    /**
        Presents key-value pairs as sorted array sorted by keys.

        :returns: A list of tuples representing the mappings in this instance.
     */
    public func toSortedArray() -> [(K, V)] {
        if isEmpty {
            return [(K, V)]()
        }
        var arr = [(K, V)]()
    ...
    private func inOrderTraversal(curNode: OrderedMapNode<K, V>, inout arr: [(K, V)]) {
        if curNode.hasLeftChild {
     ...
     */
    public func lowerBoundOfKey(key: K) -> K? {
    ...
    private func lowerBoundNodeForKey(key: K, curNode: OrderedMapNode<K, V>) -> OrderedMapNode<K, V>? {
        if key == curNode.key {
            return curNode
        }
        if key < curNode.key {
    ...
    /**
        ...
        :returns: The smallest key k in this instance such that key<k. Returns nil if no such key is found.
     */
    public func upperBoundOfKey(key: K) -> K? {
    ...
    private func upperBoundNodeForKey(key: K, curNode: OrderedMapNode<K, V>) -> OrderedMapNode<K, V>? {
        if key == curNode.key {
    ...
    private func successor(node: OrderedMapNode<K, V>) -> OrderedMapNode<K, V>? {
    ...
    private func minElement(root: OrderedMapNode<K, V>) -> OrderedMapNode<K, V> {
        var node = root
    ...
    private func maxElement(root: OrderedMapNode<K, V>) -> OrderedMapNode<K, V> {
        var node = root
    ...
    private func removeSubtreeOf(node: OrderedMapNode<K, V>?) {
        if node == nil {
            return
        }
...
class OrderedMapNode<K: Comparable, V> {
    let key: K
    let value: V
    private var _leftChild: OrderedMapNode<K, V>?
    private var _rightChild: OrderedMapNode<K, V>?
    private var _leftHeight: Int
    private var _rightHeight: Int
    private var _parent: OrderedMapNode<K, V>?
    ...
    var leftChild: OrderedMapNode<K, V>? {
        get {
            return _leftChild
        }
        set {
    ...
    var rightChild: OrderedMapNode<K, V>? {
        get {
            return _rightChild
        }
        set {
    ...
    init(key: K, value: V, parent: OrderedMapNode<K, V>?){
        self.key = key
        self.value = value
        self._parent = parent
        _leftChild = nil
